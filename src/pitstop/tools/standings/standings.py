...
from pitstop.models.standings import ConstructorStanding, DriverStanding, StandingsResponse
...
if __name__ == "__main__":
    ...
    if standings.drivers:
        ...
        for i in range(min(5, len(standings.drivers))):
            ...
            print(
                f"   {driver.position}. {driver.given_name} {driver.family_name} - {driver.points} pts ({driver.wins} wins)"
            )
    ...
    if const_standings.constructors:
        ...
        for i in range(min(3, len(const_standings.constructors))):
            ...
            print(
                f"   {constructor.position}. {constructor.constructor_name} - {constructor.points} pts"
            )
    ...
    if monaco_standings.drivers:
        ...
        print(
            f"   Leader: {monaco_standings.drivers[0].given_name} {monaco_standings.drivers[0].family_name} - {monaco_standings.drivers[0].points} pts"
        )
