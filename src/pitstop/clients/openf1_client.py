...
def query(endpoint: str, **filters) -> list[dict]:
    """Generic OpenF1 GET. Drops None filters, raises DataSourceError on failure."""
...
def get_intervals(**kw) -> list[dict]:
    return query("/intervals", **kw)


def get_pit_stops(**kw) -> list[dict]:
    return query("/pit", **kw)


def get_team_radio(**kw) -> list[dict]:
    return query("/team_radio", **kw)


def get_stints(**kw) -> list[dict]:
    return query("/stints", **kw)


def get_race_control(**kw) -> list[dict]:
    return query("/race_control", **kw)


def get_sessions(**kw) -> list[dict]:
    return query("/sessions", **kw)


def get_meetings(**kw) -> list[dict]:
    return query("/meetings", **kw)


def get_location(**kw) -> list[dict]:
    return query("/location", **kw)


def get_car_data(**kw) -> list[dict]:
    return query("/car_data", **kw)
